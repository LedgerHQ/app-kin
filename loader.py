...
bip32Path = "44'/2017'"
appFile = './bin/app.hex'
appMapFile = './debug/app.map'
iconHex = '0100000000ffffff00ffffffffffffffff8ff18ff18ff10ffe0ffe8ff18ff18ff1ffffffffffffffff'

if hasattr(sys, '_MEIPASS'):  # in pyinstaller
    appFile = os.path.join(sys._MEIPASS, 'app.hex')
    appMapFile = os.path.join(sys._MEIPASS, 'app.map')

# get data size
with open(appMapFile) as f:
    for line in f:
        match = re.match(r'^\s+(0x[0-9]+)\s+_nvram_data_size', line)
        if match:
            dataSize = match.group(1)
            break

if not dataSize:
    print('cannot determine data size!')
    sys.exit(1)
...
sys.argv = [sys.argv[0], '--appFlags', appFlags, '--path', bip32Path, '--curve', 'ed25519', '--tlv', 
